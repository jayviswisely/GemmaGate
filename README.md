...
| --- | --- | --- |
| Factual knowledge | Never guessed locally | Cheap model first, stronger only if needed |
| Mathematical reasoning | Deterministic math solver and numeric validation | Escalate only on unsupported patterns |
| Sentiment classification | Lexicon, negation, contrast, sarcasm guards | Batch uncertain label-only cases |
...
4. Accept the local answer only when validation passes.
5. Otherwise call the cheapest allowed Fireworks model.
...
On the included local gold benchmark:

```text
Accuracy: 23/23
Remote calls: 2
Remote tokens: 292
Solved fully locally: 21/23
```

These numbers are from the included benchmark set, not the hidden judging set.
They are useful for checking that routing and validation are working.
...
gemmagate/router.py
  ...
  - batches eligible sentiment residue
...
- `.env.local`, generated outputs, and Python cache files should never be
  committed.
